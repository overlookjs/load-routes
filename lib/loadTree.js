...
module.exports = function(rootGroup, groupsMap, identifyTop) {
	...
	// Load all routes
	walkTree(rootGroup, group => processSet(group, groupsMap, identifyTop));

	// Return root
	return root;
};

function processSet(aboveGroup, groupsMap, identifyTop) {
	// For each group, identify route file / route class extension to use
	const aboveRoute = aboveGroup.route,
