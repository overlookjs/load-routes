...
// Exports
class Loader {
	constructor(rootGroup, groupsMap, routeExts) {
		this.rootGroup = rootGroup;
		this.groupsMap = groupsMap;
		this.routeExts = routeExts;
	...
	process() {
		...
		// Identify root
		// TODO Allow there to be no root route file
		const {rootGroup} = this;
		const rootExt = this.getRouteExt(rootGroup);
		...
		// Load all routes
		walkTree(rootGroup, group => this.processSet(group.children));
	...
	processSet(groups) {
		...
		for (const group of groups) {
			const ext = this.getRouteExt(group);
			if (!ext) continue;
	...
	/**
	 * Find if group has a route file, and return its ext (e.g. 'route.js').
	 * @param {Object} group - Group
	 * @returns {string|undefined} - Ext of route file (undefined if none found)
	 */
	getRouteExt(group) {
		const {files} = group;
		return this.routeExts.find(ext => files[ext]);
	}
}

function loadTree(rootGroup, groupsMap, exts) {
	const loader = new Loader(rootGroup, groupsMap, exts);
